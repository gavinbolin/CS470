...
public class theRobot extends JFrame {
    ...
    void updateProbabilities(int action, String sonars) {
        ...
        for (int y = 0; y < mundo.height; y++) {
            for (int x = 0; x < mundo.width; x++) {
                if (mundo.grid[x][y] == 0 || mundo.grid[x][y] == 3) {
                    probs[x][y] = transition(x,y,action);
                    probs[x][y] = sensor(x,y,sonars);
                    total += probs[x][y];
                } // track red spots (2) // ability to know if pushing into a wall
            }
        }
        normalize(total);
    ...
    double transition(int x, int y, int action){
        double currProb = probs[x][y];
        double prevProb;
        double fmove = (1-moveProb)/4;
        //int blc = 0;
        double blc_up = 0;
        double blc_dn = 0;
        double blc_rt = 0;
        double blc_lt = 0;
        if(mundo.grid[x][y+1] == 1 || mundo.grid[x][y+1] == 2){blc_up = probs[x][y];}
        if(mundo.grid[x][y-1] == 1 || mundo.grid[x][y-1] == 2){blc_dn = probs[x][y];}
        if(mundo.grid[x-1][y] == 1 || mundo.grid[x-1][y] == 2){blc_rt = probs[x][y];}
        if(mundo.grid[x+1][y] == 1 || mundo.grid[x+1][y] == 2){blc_lt = probs[x][y];}
        if(action == NORTH){prevProb = probs[x][y+1];}
        else if(action == SOUTH){prevProb = probs[x][y-1];}
        else if(action == EAST){prevProb = probs[x-1][y];}
        else if(action == WEST){prevProb = probs[x+1][y];} // && mundo.grid[x+1][y] != 2
        else{prevProb = (currProb + blc_up + blc_dn + fmove*blc_rt + fmove*blc_lt);}
        return((moveProb * (prevProb)) + (fmove * currProb));
    }

    double sensor(int x, int y, String sonars){
        int correct = 0;
        if (sonars.charAt(0) == mundo.grid[x][y-1]){correct++;}
        if (sonars.charAt(1) == mundo.grid[x][y+1]){correct++;}
        if (sonars.charAt(2) == mundo.grid[x+1][y]){correct++;}
        if (sonars.charAt(3) == mundo.grid[x-1][y]){correct++;}
        return (Math.pow(sensorAccuracy, correct) * Math.pow((1-sensorAccuracy), (4 - correct))) * probs[x][y];
    }

    void normalize(double total){
        for (int y = 0; y < mundo.height; y++) {
            for (int x = 0; x < mundo.width; x++) {
               probs[x][y] = probs[x][y]/total;
            }
        }
    }
